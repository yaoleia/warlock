...
proxy.on('proxyRes', (proxyRes, req, res) => {
    console.log(`Got response from proxy, url=${req.url}, code=${proxyRes.statusCode}, msg=${proxyRes.statusMessage}`);
});
...
function _webAsync(req, res, opts) {
    ...
    return new Promise(resolve => {
        proxy.web(req, res, opts, err => {
            if (err) {
                console.error(`Error occurred while proxying request ${req.url} to ${opts.target}`, err);
            }
            resolve(err);
        });
    });
